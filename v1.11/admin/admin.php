...
class ZarzadzajProduktami {
    ...
    public function DodajProdukt() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $stmt = $this->db->prepare("INSERT INTO produkty (tytul, opis, data_utworzenia, data_modyfikacji, 
                data_wygasniecia, cena_netto, vat, ilosc, status, kategoria_id, gabaryt, zdjecie) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            if ($stmt->execute()) {
                ...
                echo "<p>Wystąpił błąd podczas dodawania produktu.</p>";
            }
        }
    ...
    public function EdytujProdukt($id) {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $stmt = $this->db->prepare("UPDATE produkty SET tytul = ?, opis = ?, data_modyfikacji = ?, 
                data_wygasniecia = ?, cena_netto = ?, vat = ?, ilosc = ?, status = ?, 
                kategoria_id = ?, gabaryt = ?, zdjecie = ? WHERE id = ? LIMIT 1");
            
            $stmt->bind_param("ssssddisssssi", $tytul, $opis, $data_modyfikacji, $data_wygasniecia, 
                $cena_netto, $vat, $ilosc, $status, $kategoria_id, $gabaryt, $zdjecie, $id);
        ...
        <form method='POST' enctype='multipart/form-data'>
            ...
            <label>Status dostępności: 
                <select name='status' required>
                    <option value='dostepny' " . ($produkt['status'] == 'dostepny' ? 'selected' : '') . ">Dostępny</option>
                    <option value='niedostepny' " . ($produkt['status'] == 'niedostepny' ? 'selected' : '') . ">Niedostępny</option>
                    <option value='oczekujacy' " . ($produkt['status'] == 'oczekujacy' ? 'selected' : '') . ">Oczekujący</option>
                </select>
            </label><br>
            <label>Kategoria: 
                <select name='kategoria' required>
    ...
    //    echo "<label style='margin-bottom: 5px; font-weight: bold;'>Szukaj:</label>";
       ...
       echo "</div>";
        ...
        // Filtr statusu
        echo "<select name='status' style='padding: 8px;'>";
        echo "<option value=''>Wszystkie statusy</option>";
        echo "<option value='dostepny'" . (($_GET['status'] ?? '') === 'dostepny' ? ' selected' : '') . ">Dostępne</option>";
        echo "<option value='niedostepny'" . (($_GET['status'] ?? '') === 'niedostepny' ? ' selected' : '') . ">Niedostępne</option>";
        echo "<option value='oczekujacy'" . (($_GET['status'] ?? '') === 'oczekujacy' ? ' selected' : '') . ">Oczekujące</option>";
        echo "</select>";
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $status_style = match ($dostepnosc) {
                'Dostępny' => 'color: #28a745; background: #e8f5e9; padding: 4px 8px; border-radius: 4px;',
                'Brak w magazynie' => 'color: #dc3545; background: #fbe9e7; padding: 4px 8px; border-radius: 4px;',
                'Niedostępny' => 'color: #dc3545; background: #fbe9e7; padding: 4px 8px; border-radius: 4px;',
                'Wygasły' => 'color: #ffa000; background: #fff3e0; padding: 4px 8px; border-radius: 4px;',
                default => 'color: #666; background: #f5f5f5; padding: 4px 8px; border-radius: 4px;'
            };
            ...
            echo "<tr style='border-bottom: 1px solid #dee2e6;'>
                ...
                <td style='padding: 12px; text-align: center;'><span style='{$status_style}'>" . $dostepnosc . "</span></td>
